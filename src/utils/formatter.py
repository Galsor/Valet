...
from src.utils.type import MessageList, Message
    ...
    if isinstance(txt, str):
        ...
        for item in txt:
            if isinstance(item, str):
                ...
                raise NotImplementedError(f"No parser implemented for {type(item)} submessage")
        return raw_txt
    else:
...
def format_messages(conversations: MessageList) -> Dict[int, str]:
    ...
    for message in conversations:
        ...
        # Add ref to question
        if 'reply_to_message_id' in message and message.get("reply_to_message_id") in conv_mapping:
            responded_message = conv_mapping[message["reply_to_message_id"]]
            message["context"] = format_context_message(responded_message)
        
        # delete unnecessary keys
        for key in ['type', 'date_unixtime', 'from_id',"reply_to_message_id", 'text_entities', "edited", "edited_unixtime"]:
            if key in message:
                del message[key]
